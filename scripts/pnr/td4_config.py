...
PRI_CELL = "FILL2"             # TAP 直後に予約する優先 M2 コリドー
PRI_W = 10.8
...
# GDS のセル原点は prBoundary の左下ではない（REG8x16 は (-86.4, -54.6)）。
# 配置するときはこのオフセットを足す。実測値。
MACRO_ORIGIN_OFF = (86.4, 54.6)
...
# 面積の損は無い）。
#
# place.py の見積り（ネット交差数 x 5.4 µm）は [140, 173, 173, 146, 38, 0]。
# ルータのジョグは行またぎ 1 本ごとに新しいトラックを取るので見積りでは足りない
# （SCLK_SPI は見積りの数倍を積んで最後に圧縮した）。ここでは約 1.3 倍 + 余白。
# 開口 1840 − 行 297 = 1543 µm がチャネルに使える上限で、下の合計は 1140。
# **足りなければここを増やす**（403 µm 残してある）。
CH_HEIGHTS = [260.0, 220.0, 220.0, 200.0, 120.0, 120.0]
