...
def draw(ax, d, step):
    ...
    ax.add_patch(Rectangle((0, 0), cw, ch, fc="#fcfcfc", ec="#2c3e50", lw=1.4))
    # channels
    y = 0.0
    ...
    # the macro's 21 signal pins are all on its bottom edge (exaggerated)
    ax.add_patch(Rectangle((mx0, my0), mx1 - mx0, 14.0, fc="#8e44ad", ec="none"))
    ax.set_xlim(-40, cw + 40)
    ax.set_ylim(-40, ch + 40)
    ax.set_aspect("equal")
...
def main(out=None):
    ...
    fig.suptitle(f"TD4 placement  --  core {cw:.1f} x {ch:.1f} um "
                 f"(frame opening {hi-lo:.0f} um, margin {hi-lo-ch:.0f} um)   "
    ...
    fig.text(0.5, 0.015,
             ...
             "orange = REG8x16 macro   purple = its 21 signal pins "
             "(all on the bottom edge, drawn thick)",
