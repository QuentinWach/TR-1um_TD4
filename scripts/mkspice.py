#!/usr/bin/env python3
"""REG4x16 の LVS 用ソースネットリスト（設計意図側）を生成する。

  usage: python3 scripts/mkspice.py -o spice/REG4x16_src.spi
...
def top():
    ...
    o = [f"* REG4x16 — {BITS}bit x {WORDS}word レジスタファイル (248.4 x 933.0 um)",
         "* ポート名は GDS のピンラベル実測に一致させてある",
         f".subckt REG4x16 {apins} {P_WEB} {dpins} {qpins} {VDD} {VSS}", ""]
    ai = " ".join(f"a{k} ab{k}" for k in range(4))
    o.append(f"* アドレス相補生成 + WE バッファ")
    o.append(f"XADDBUF {apins} {P_WEB} {ai} web {VDD} {VSS} ADDBUF")
    o.append("")
    ...
    for k in range(WORDS // 2):
        ...
        o.append(f"XDEC2_{k} a0 ab0 {hi} web "
    ...
    o.append(".ends REG4x16")
    return o


HEADER = """* REG4x16 — LVS ソースネットリスト（設計意図）
...
def main():
    ...
    ap.add_argument("-o", "--out", default="spice/REG4x16_src.spi")
    a = ap.parse_args()
    lines = [HEADER]
    ...
    print(f"wrote {a.out}")
    print(f"  サブサーキット: TLAT / DEC2 / REGBUF / ADDBUF / REG4x16")
